...
class PipelinesListController:
    ...
    def __init__(self, pipelines_list: PipelinesList):
        ...
        self.pipelines_model = (
            self.core.pipelines_manager.pipeline_data_model if self.core else None
        )
        self.item_widget_list = list()

        self._connect_controller()
        if self.core and self.pipelines_model:
    ...
    def _connect_to_events(self):
        ...
        pipelines_model_event_bus = (
            self.core.event_bus.pipeline_manager_event_bus.pipeline_data_model_event_bus
        )
    ...
    def _disable_interactions(self):
        self.pipelines_list.new_pipeline_toolButton.setEnabled(False)
        self.pipelines_list.pipelines_list_listWidget.setEnabled(False)
        self.pipelines_list.pipelines_list_listWidget.clear()


    def update_list_widget(self):
        if not self.pipelines_model:
            return
        self.pipelines_list.pipelines_list_listWidget.clear()
        self.item_widget_list.clear()

        active_pipeline = self.core.pipelines_manager.active_pipeline if self.core else None
    ...
    def _add_list_item(self, item_name: str, is_active: bool=False):
        ...
        if self.core:
            ...
            connection = self.core.event_bus.pipeline_manager_event_bus.activePipelineChanged.connect(_on_active_unit_changed)  # type: ignore[attr-defined]
    ...
    def _on_remove_button_clicked(self, pipeline_name: str):
        if not pipeline_name is None and self.pipelines_model:
            try:
    ...
    def _on_new_pipeline_button_clicked(self):
        ...
        if result and self.pipelines_model and self.pipelines_model.initialized:
