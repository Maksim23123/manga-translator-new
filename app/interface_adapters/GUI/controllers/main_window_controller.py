...
from application.project.dto import CreateProjectRequest, SaveProjectRequest, LoadProjectRequest
...
class MainWindowController:
    ...
    def on_save_project_triggered(self) -> None:
        if save_location := self._presenter.request_save_location_uri():
            try:
                req = SaveProjectRequest(save_location)
                response = self._save_project_use_case.execute(req)
                log.info(f"Project saved to: {response.access_uri}")
            except Exception as ex:
                log.error(ex)
    
    
    def on_load_project_triggered(self) -> None:
        if load_location := self._presenter.request_load_location_uri():
            try:
