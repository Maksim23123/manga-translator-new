...
- Cleanup rules: deleting a pipeline removes only its draft graph (final stays on disk for recovery). The next load runs the orphan sweep and resets PyFlow to a blank graph.
